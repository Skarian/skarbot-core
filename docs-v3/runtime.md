...
- `web_search` finds candidate URLs and lightweight results
- `web_fetch` retrieves one URL and extracts readable content without requiring a full browser surface
...
- `model(list)` returns the configured available models
...
- `schedule()` lists the user’s existing schedules and their stable handles
- `schedule(set, at)` creates a one-off schedule
- `schedule(set, cron)` creates a recurring schedule
- `schedule(edit, id, at)` rewrites an existing schedule as a one-off schedule
- `schedule(edit, id, cron)` rewrites an existing schedule as a recurring schedule
- `schedule(delete, id)` removes an existing schedule
- schedule handles are stable human-readable names unique per user
- one-off schedules create one-off throwaway task threads
- recurring schedules bind to one dedicated reusable task thread
...
Approvals are deterministic runtime objects, not model guesses. Approval record storage and shape live in [Architecture](architecture.md).
...
- the approval record status becomes `feedback`
