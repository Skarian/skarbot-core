...
- **Let placement and filesystem metadata carry state.** Prefer directory placement, filenames, and file timestamps over duplicated status or timestamp fields.
...
Skarbot owns the assistant product, the runtime state under `~/skarbot/state`, the execution workspaces under `~/skarbot/workspaces`, and the small host-managed deployment files under `~/.skarbot`.
